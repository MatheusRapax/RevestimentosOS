...
export class ModuleGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    const hasModule = requiredModules.some((module) =>
      clinicModules.includes(module),
    );

    if (!hasModule) {
      console.error(
        `❌ ModuleGuard: Access denied for ${user.id} to modules ${requiredModules.join(', ')}. Active modules: ${clinicModules.join(', ')}`,
      );
      throw new ForbiddenException(
        `Module ${requiredModules.join(', ')} is disabled for this tenant`,
      );
    }

    console.log(
      `✅ ModuleGuard: Access granted for modules ${requiredModules.join(', ')}`,
    );
    return true;
  }
}
