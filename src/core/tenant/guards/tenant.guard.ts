...
export class TenantGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    try {
      ...
      console.log(
        `🔍 TenantGuard: Validating access for user ${request.user?.id} to clinic ${clinicId}`,
      );
      ...
      if (!clinic) {
        console.error(
          `❌ TenantGuard: Access denied for user ${request.user.id} to clinic ${clinicId}`,
        );
        throw new ForbiddenException(
      ...
      console.log(
        `✅ TenantGuard: Access granted to ${clinic.name} (${clinic.id})`,
      );
