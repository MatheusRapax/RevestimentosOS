...
export class SchedulingService {
  ...
  async create(clinicId: string, createAppointmentDto: CreateAppointmentDto) {
    ...
    const { outsideHours, reason } = await this.isOutsideWorkingHours(
      clinicId,
      start,
      startTimeStr,
      endTimeStr,
    ...
    await this.validateProfessionalWorkingHours(
      clinicId,
      professionalId,
      start,
      startTimeStr,
  ...
  private async validateProfessionalWorkingHours(
    ...
    const profStartTime =
      workingHours?.startTime ?? (isWeekend ? null : '08:00');
  ...
  async updateWorkingHours(
    ...
    for (const day of data) {
      ...
      if (day.isOpen && (!day.startTime || !day.endTime)) {
        throw new BadRequestException(
          'startTime e endTime são obrigatórios quando aberto',
        );
      }
