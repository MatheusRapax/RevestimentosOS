...
export class DeliveriesService {
  ...
  async create(clinicId: string, createDto: CreateDeliveryDto) {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      // Update order status to EM_SEPARACAO when a delivery is scheduled
      try {
        // Here we call the existing logic from the service context, but
        // we should do this directly via transaction to avoid isolation issues
        await tx.order.update({
          where: { id: createDto.orderId },
          data: { status: OrderStatus.EM_SEPARACAO },
        });
      } catch (error) {
        console.error(
          `[Delivery Create] Failed to update order ${createDto.orderId} to EM_SEPARACAO:`,
          error,
        );
      }
  ...
  async update(id: string, clinicId: string, updateDto: UpdateDeliveryDto) {
    ...
    if (
      ...
      orderStatus !== OrderStatus.SAIU_PARA_ENTREGA
    ) {
      ...
      try {
        ...
        console.error(
          `[Delivery→Order] Failed to update order ${orderId} to SAIU_PARA_ENTREGA:`,
          error,
        );
      }
    }
    ...
    if (newStatus === 'DELIVERED' && orderStatus !== OrderStatus.ENTREGUE) {
      ...
      try {
        ...
        console.log(
          `[Delivery→Order] Order ${orderId} → ENTREGUE (auto-finalized)`,
        );
      } catch (error) {
        console.error(
          `[Delivery→Order] Failed to update order ${orderId} to ENTREGUE:`,
          error,
        );
      }
    }
