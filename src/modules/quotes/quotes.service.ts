...
export class QuotesService {
  ...
  async create(
    ...
    if (createQuoteDto.discountPercent && createQuoteDto.discountPercent > 0) {
      discountCents = Math.round(
        subtotalCents * (createQuoteDto.discountPercent / 100),
      );
    }

    const deliveryFee =
      createQuoteDto.deliveryFeeCents || createQuoteDto.deliveryFee || 0;
  ...
  async checkAvailability(id: string, clinicId: string) {
    ...
    const itemsWithAvailability = await Promise.all(
      quote.items.map(async (item) => {
        ...
        const productStock = await this.stockService.findOne(
          item.productId,
          clinicId,
        );
    ...
    // Summary status
    const allAvailable = itemsWithAvailability.every(
      (i) => i.status === 'AVAILABLE',
    );
    const anyPartial = itemsWithAvailability.some(
      (i) => i.status === 'PARTIAL',
    );
  ...
  async approveQuote(id: string, clinicId: string) {
    ...
    if (quote.status !== QuoteStatus.AGUARDANDO_APROVACAO) {
      throw new BadRequestException(
        'Apenas orçamentos enviados podem ser aprovados',
      );
    }
  ...
  async convertToOrder(id: string, clinicId: string, sellerId: string) {
    ...
    if (quote.status !== QuoteStatus.APROVADO) {
      throw new BadRequestException(
        'Apenas orçamentos aprovados podem ser convertidos em pedido',
      );
    }
    ...
    const order = await this.prisma.$transaction(async (tx) => {
      ...
      for (const res of quoteReservations) {
        ...
        const matchingOrderItem = newOrder.items.find(
          (oi) =>
  ...
  async reserveStock(id: string, clinicId: string) {
    ...
    if (
      quote.status !== QuoteStatus.EM_ORCAMENTO &&
      quote.status !== QuoteStatus.AGUARDANDO_APROVACAO
    ) {
      throw new BadRequestException(
        'Apenas orçamentos em Rascunho ou Enviados podem ter estoque reservado',
      );
    }
    ...
    const existingReservations =
      await this.stockReservationsService.findByQuote(clinicId, id);

    const results = [];

    for (const item of quote.items) {
      // Check what is already reserved for this specific item
      const itemReservations = existingReservations.filter(
        (r: any) => r.quoteItemId === item.id,
      );
      const alreadyReserved = itemReservations.reduce(
      ...
      if (remainingToReserve > 0 && !item.preferredLotId) {
        ...
        lots.sort((a: any, b: any) => {
          const availA =
            a.quantity -
            (a.reservations?.reduce(
              (sum: number, r: any) => sum + r.quantity,
              0,
            ) || 0);
          const availB =
            b.quantity -
            (b.reservations?.reduce(
              (sum: number, r: any) => sum + r.quantity,
              0,
            ) || 0);
      ...
      results.push({
        ...
        requested: item.quantityBoxes,
        newlyReserved:
          item.quantityBoxes - alreadyReserved - remainingToReserve,
      });
    }
  ...
  private async recalculateQuoteTotals(quoteId: string) {
    ...
    const totalCents = Math.max(
      0,
      subtotalCents - discountCents + quote.deliveryFee,
    );
  ...
  async removeItem(id: string, itemId: string, clinicId: string) {
    ...
    if (quote.status !== QuoteStatus.EM_ORCAMENTO) {
      throw new BadRequestException(
        'Apenas orçamentos em rascunho podem ter itens removidos',
      );
    }
  ...
  async updateItem(
    ...
    itemId: string,
    clinicId: string,
    dto: import('./dto/update-quote-item.dto').UpdateQuoteItemDto,
  ) {
    const quote = await this.findOne(id, clinicId);

    if (quote.status !== QuoteStatus.EM_ORCAMENTO) {
      throw new BadRequestException(
        'Apenas orçamentos em rascunho podem ter itens editados',
      );
    }
    ...
    if (processed.quantityBoxes < currentItem.quantityBoxes) {
      ...
      const currentlyReserved = activeReservations.reduce(
