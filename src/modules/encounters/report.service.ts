  ...
  private async createPDF(encounter: any): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      ...
      doc
        .fontSize(20)
        .font('Helvetica-Bold')
        .text('RELATÓRIO CLÍNICO', { align: 'center' });
      doc.moveDown(0.5);
      doc
        .fontSize(12)
        .font('Helvetica')
        .text(encounter.clinic.name, { align: 'center' });
      doc.moveDown(1);
      ...
      if (encounter.procedures.length > 0) {
        ...
        encounter.procedures.forEach((proc: any, index: number) => {
          ...
          doc.text(
            `${index + 1}. ${proc.name} - ${date.toLocaleDateString('pt-BR')}`,
          );
          if (proc.notes) {
      ...
      if (encounter.consumables.length > 0) {
        ...
        encounter.consumables.forEach((cons: any, index: number) => {
          doc.text(
            `${index + 1}. ${cons.product.name} - Qtd: ${cons.quantity} ${cons.product.unit || 'un'}`,
          );
        });
        doc.moveDown(1);
      }
      ...
      if (encounter.attachments.length > 0) {
        ...
        encounter.attachments.forEach((att: any, index: number) => {
          ...
          doc.text(
            `${index + 1}. ${att.originalName} (${att.mimeType}) - ${size}`,
          );
        });
        doc.moveDown(1);
      }
      ...
      const now = new Date();
      doc.text(`Gerado em: ${now.toLocaleString('pt-BR')}`, {
        align: 'center',
      });
