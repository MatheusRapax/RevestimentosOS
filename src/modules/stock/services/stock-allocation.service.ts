...
export class StockAllocationService {
  ...
  async autoAllocateOrder(clinicId: string, orderId: string, userId?: string) {
    ...
    for (const item of order.items) {
      ...
      const reservedQty = itemReservations.reduce(
      ...
      if (suitableLot) {
        ...
        if (totalAvailable >= neededQty) {
          ...
          allReserved = false;
          this.logger.warn(
            `Order ${orderId}: Item ${item.product.name} needs ${neededQty} but no single lot has enough. Total available: ${totalAvailable}. Skipping (Lot Integrity Rule).`,
          );
        } else {
          allReserved = false;
          this.logger.warn(
            `Order ${orderId}: Item ${item.product.name} insufficient stock.`,
          );
        }
      }
    }
    ...
    if (order.status === OrderStatus.PAGO) {
      ...
      if (nextOrderStatus) {
        ...
        this.logger.log(
          `Auto-updated order ${orderId} status to ${nextOrderStatus}`,
        );
      }
    }
  ...
  async processStockArrival(
    ...
    this.logger.log(
      `Stock Arrival: Found ${orders.length} orders waiting for products ${productIds.join(', ')}`,
    );
    ...
    for (const order of orders) {
      try {
        ...
        this.logger.error(
          `Failed to auto-allocate order ${order.id} on stock arrival`,
          error,
        );
      }
    }
