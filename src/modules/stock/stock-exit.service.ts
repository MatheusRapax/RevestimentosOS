...
export class UpdateStockExitItemDto {
  quantity?: number;
  lotId?: string;
}
...
export class StockExitService {
  ...
  async createFromOrder(clinicId: string, orderId: string, userId: string) {
    ...
    reservations.forEach((r) => {
      if (r.lot) {
        ...
        if (!reservationsByProduct.has(prodId))
          reservationsByProduct.set(prodId, []);
    ...
    for (const orderItem of order.items) {
      const productReservations =
        reservationsByProduct.get(orderItem.productId) || [];
      ...
      // Fix: Stock is controlled by unit/box, not by resulting area (e.g. m2).
      const quantity = orderItem.quantityBoxes;
  ...
  async addItem(clinicId: string, exitId: string, dto: AddStockExitItemDto) {
    ...
    if (!exit || exit.status !== ExitStatus.DRAFT)
      throw new BadRequestException('Saída inválida');
    ...
    const totalStock =
      product?.lots.reduce((acc, lot) => acc + lot.quantity, 0) || 0;
  ...
  async updateItem(
    ...
    exitId: string,
    itemId: string,
    dto: UpdateStockExitItemDto,
  ) {
    const exit = await this.prisma.stockExit.findUnique({
      where: { id: exitId, clinicId },
    });
    if (!exit) throw new NotFoundException('Saída não encontrada');
    if (exit.status !== ExitStatus.DRAFT)
      throw new BadRequestException(
        'Apenas rascunhos podem ter itens editados',
      );

    return this.prisma.stockExitItem.update({
      where: { id: itemId, stockExitId: exitId },
      data: {
  ...
  async confirmExit(clinicId: string, exitId: string, userId: string) {
    ...
    if (exit.status !== ExitStatus.DRAFT)
      throw new BadRequestException(
        'Esta saída já foi confirmada e não pode ser processada novamente.',
      );
    if (exit.items.length === 0)
      throw new BadRequestException(
        'A saída não possui itens. Adicione produtos antes de confirmar.',
      );
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      for (const item of exit.items) {
        ...
        if (totalAvailable < remainingQty) {
          throw new BadRequestException(
            `Estoque insuficiente para o item (Lote/Produto).`,
          );
        }
  ...
  async cancelExit(clinicId: string, exitId: string) {
    ...
    if (!exit || exit.status !== ExitStatus.DRAFT)
      throw new BadRequestException('Apenas rascunhos podem ser cancelados.');
  ...
  async deleteExit(clinicId: string, exitId: string) {
    const exit = await this.prisma.stockExit.findUnique({
      where: { id: exitId, clinicId },
    });
    if (!exit) {
