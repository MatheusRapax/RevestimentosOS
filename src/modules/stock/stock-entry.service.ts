...
export class StockEntryService {
    ...
    if (existingEntry) {
      if (existingEntry.clinicId !== clinicId) {
        throw new BadRequestException(
          'Esta nota fiscal já foi importada em outra clínica.',
        );
      }
      if (existingEntry.status !== EntryStatus.DRAFT) {
        throw new BadRequestException(
          `Nota fiscal já importada e com status: ${existingEntry.status === 'CONFIRMED' ? 'CONFIRMADA' : existingEntry.status}. Verifique o histórico.`,
        );
      }
  ...
  async update(clinicId: string, entryId: string, dto: UpdateStockEntryDto) {
    ...
    return this.prisma.stockEntry.update({
      ...
      data: {
        ...
        // --- Fiscal Data ---
        accessKey: dto.accessKey === '' ? null : dto.accessKey,
        operationNature: dto.operationNature,
        protocol: dto.protocol,
        model: dto.model,
  ...
  async addItem(clinicId: string, entryId: string, dto: AddStockEntryItemDto) {
    ...
    if (!entry) throw new NotFoundException('Entrada não encontrada');
    if (entry.status !== EntryStatus.DRAFT)
      throw new BadRequestException('Entrada já confirmada ou cancelada');
  ...
  async updateItem(
    ...
    if (entry.status !== EntryStatus.DRAFT)
      throw new BadRequestException(
        'Apenas rascunhos podem ter itens editados',
      );

    const existingItem = await this.prisma.stockEntryItem.findUnique({
      where: { id: itemId, stockEntryId: entryId },
    });
    if (!existingItem) throw new NotFoundException('Item não encontrado');

    const newQuantity = dto.quantity ?? existingItem.quantity;
    const newUnitCost =
      dto.unitCost !== undefined ? dto.unitCost : existingItem.unitCost;
    const newTotalCost =
      newUnitCost !== null ? newQuantity * newUnitCost : null;

    const item = await this.prisma.stockEntryItem.update({
      where: { id: itemId, stockEntryId: entryId },
      data: {
        ...
        ...(dto.cst !== undefined && { cst: dto.cst }),
        ...(dto.discountValueCents !== undefined && {
          discountValueCents: dto.discountValueCents,
        }),
        ...(dto.freightValueCents !== undefined && {
          freightValueCents: dto.freightValueCents,
        }),
        ...(dto.insuranceValueCents !== undefined && {
          insuranceValueCents: dto.insuranceValueCents,
        }),
        ...(dto.valueICMS !== undefined && { valueICMS: dto.valueICMS }),
        ...(dto.rateICMS !== undefined && { rateICMS: dto.rateICMS }),
        ...(dto.valueIPI !== undefined && { valueIPI: dto.valueIPI }),
        ...(dto.rateIPI !== undefined && { rateIPI: dto.rateIPI }),
      },
    });
  ...
  async removeItem(clinicId: string, entryId: string, itemId: string) {
    ...
    if (!entry || entry.status !== EntryStatus.DRAFT)
  ...
  private async updateEntryTotal(entryId: string) {
    const items = await this.prisma.stockEntryItem.findMany({
  ...
  async confirmEntry(clinicId: string, entryId: string, userId: string) {
    ...
    if (!entry) throw new NotFoundException('Entrada não encontrada');
    if (entry.status !== EntryStatus.DRAFT)
      throw new BadRequestException('Entrada não está em rascunho');
    if (entry.items.length === 0)
      throw new BadRequestException(
        'A entrada não possui itens. Adicione produtos antes de confirmar.',
      );
    ...
    if (entry.type === EntryType.INVOICE) {
      ...
      const missingFields = requiredFields
        .filter((req) => {
          const val = entry[req.field as keyof typeof entry];
    ...
    if (entry && entry.items) {
      ...
      try {
        await this.stockAllocationService.processStockArrival(
          clinicId,
          productIds,
          userId,
        );
  ...
  async cancelEntry(clinicId: string, entryId: string) {
    ...
    if (entry.status !== EntryStatus.DRAFT)
      throw new BadRequestException(
        'Esta entrada já foi processada e não pode ser cancelada. Realize um ajuste de estoque se necessário.',
      );
  ...
  async deleteEntry(clinicId: string, entryId: string) {
    ...
    if (entry.status !== EntryStatus.DRAFT)
      throw new BadRequestException(
        'Não é possível excluir uma entrada já confirmada.',
      );
