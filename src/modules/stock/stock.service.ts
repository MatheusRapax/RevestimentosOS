...
export class StockService {
  ...
  async findOne(id: string, clinicId: string) {
    ...
    const totalReserved = product.lots.reduce((sum, lot) => {
      return (
        sum +
        lot.reservations.reduce((rSum: number, r: any) => rSum + r.quantity, 0)
      );
    }, 0);
  ...
  async adjustStock(clinicId: string, dto: AdjustStockDto) {
    ...
    if (dto.lotId) {
      ...
      if (newQuantity < 0) {
        throw new BadRequestException(
          'Estoque do lote não pode ficar negativo',
        );
      }
      ...
      if (dto.quantity < 0) {
        ...
        await this.prisma.$transaction(async (tx) => {
          ...
          if (totalAvailable < remainingToRemove) {
            throw new BadRequestException(
              'Estoque insuficiente para ajuste negativo',
            );
          }
  ...
  async getShadeCaliberAlerts(clinicId: string) {
    ...
    for (const product of products) {
      ...
      const uniqueShades = [
        ...new Set(lotsWithShade.map((l) => l.shade)),
      ].filter(Boolean);
      ...
      const uniqueCalibers = [
        ...new Set(lotsWithCaliber.map((l) => l.caliber)),
      ].filter(Boolean);
