...
async function main() {
    ...
    try {
        ...
        // 8. Finance (Boletos)
        console.log('8. Finance: Generating Invoice...');
        const dueDate = new Date();
        dueDate.setDate(dueDate.getDate() + 5);

        const invoiceRes = await fetch(`${BASE_URL}/finance/invoices`, {
            method: 'POST',
            headers,
            body: JSON.stringify({
                orderId: order.id,
        ...
        if (!invoiceRes.ok) throw new Error(`Invoice generation failed: ${await invoiceRes.text()}`);
        const invoice = await invoiceRes.json();
        console.log(green(`✅ Invoice Generated: ${invoice.id}`));
        console.log(`   Amount: ${invoice.amountCents} cents`);
        console.log(`   BarCode: ${invoice.barCode}`);

        // Update Status
        console.log('8.1 Updating Invoice Status to PAID...');
        const patchRes = await fetch(`${BASE_URL}/finance/invoices/${invoice.id}/status`, {
            method: 'PATCH',
            headers,
            body: JSON.stringify({ status: 'PAID' })
        });

        if (!patchRes.ok) throw new Error(`Invoice update failed: ${await patchRes.text()}`);
        const updatedInv = await patchRes.json();
        console.log(green(`✅ Invoice Status: ${updatedInv.status}, PaidAt: ${updatedInv.paidAt}`));
